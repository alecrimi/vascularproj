function   [res, dens, vol] = extract_feat_par(fname, data,   method, use_snake, n_block, tot_blocks) %, min_area_flags )
...
%use_snake=0;
%fname
%fname = 'Z:\data\a.reuss\200108_MB_APPPS1_Tg_6mo_vessels\output\test\hFTAA_100_SMA-Cy3_200_Pdxl-647_200_X22045_Y66262_488_nm_515LP_1x_Both_000000.tif';
%tot_blocks = 1;
...
foreground =  imbinarize(data,0.1); 
...
res_pre_watershed = max(max(max(bwlabeln(bw))))
min_area = 10; %Plaques can be of a minimum number of pixels otherwise they are artefacts
%Not Used:
max_area = 1500; %Plaques can be of a maximum number of pixels otherwise they are artefacts
...
bw4 = bw4 - bwareaopen(bw4, max_area);
...
%saveastiff(uint8(bw3), [fname  num2str(n_block) '_seg_ws_preopening.tif']);
